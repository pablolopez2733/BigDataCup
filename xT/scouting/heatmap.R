...
frame <- read.csv("scouting/frame_plot.csv")%>%
  mutate(
    x = (x/10)-100,
    y = (y/10)-42.5
  ); color_lines = "white"
...
xt_viz <- ggplot() +
  ...
  coord_fixed() + 
  labs(
    title= 'Distribution of expected threat (xT) across rink - Scouting',
    fill = 'xT'
  ); xt_viz + ggsave('plots/scouting.png',width=7,height=7,dpi=700)
